...
function displayDataTable( $_total_data, $_data, $config, $k_groups = 5){
	...
	// Títulos de las columnas en la tabla
	...
	$html = '<table id="responsiveTable" class="table table-striped table-bordered no-margin">';
					...
					if(!empty($actionButtons)){
						...
						$html .= '<td align="center">';
							foreach($actionButtons as $actionButton){
								...
								//Accion eliminar con validación
								if($actionButton["alt"] == 'Eliminar'){
									 $confirmar_elimniar = 'onclick = "return Confirmar()"';
									}					
								foreach($actionButton['hrefReplaceArr'] as $replace){
									$href = str_replace($replace['search'], $data[$replace['field_replace']],$href);
								}
								$html .= '<a href="' . $href . '" '.$target. $confirmar_elimniar.' >
										  <img border="0" src="' . $actionButton['img'] . '" alt="' . $actionButton['alt'] . '" title="' . $actionButton['title'] . '"  style="padding-right:10px; padding-left:5px;"/>
										  </a>';
							}		 
							unset($confirmar_elimniar);
						$html .= '</td>';
					}
				$html .= '</tr>';
			}
...
function display( $_total_data, $_data, $config, $k_groups = 5){
	...
	<div>
		<table>
			<tr>
				<td>
					<input type="text" id="qtmp" value="' . $q . '" /> 
...
if( empty($_data) ){
		$html .= '<div>
					<b>No hay resultados encontrados</b>
					<br />'. printPager('formPager', $hiddenFields, 'paginacion','paginacion paginacionActual','', $k_groups ) . '				
				  </div>';
	}else{
				
	$html .= '
	<div>
	<table border="0" width="100%" cellpadding="2" cellspacing="2" bgcolor="#CCCCCC" class="listPanel">
	';
