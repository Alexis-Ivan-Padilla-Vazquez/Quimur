...
//ITEMS FUNCTIONS
	...
	function MCart_addItem($itemId,$quantity=1,$des, $tmp, $etd){
		...
		if( MCart_ItemExists($itemId) ){
			...
			if(!empty($productoObj)){
				...
				$item['descripcion'] = $productoObj['descripcion'];
...
//PURCHASE FUNCTIONS
	...
	function MCart_Purchase_CreateOrder($metodo_pago, $comments){
		...
		$content = 'Se ha registrado su cotizaci&oacute;n  exitosamente puede ver los detalles actualizados de su cotizaci&oacute;n en cualquier momento desde el 
		...
		Acontinuacion le presentamos las caracteristicas de su cotizaci&oacute;n:<br />
		';
		
		$content_admin = 'Se ha generado una nueva cotizaci&oacute;n  puede modificar el estatus de esta cotizaci&oacute;n desde el siguiente link de Acceso: 
		...
		Acontinuacion le presentamos las caracteristicas de la cotizaci&oacute;n:<br />
		';
		...
		ob_end_clean();
			lib_sendEmail($user['email'],'Nueva Cotizacion No. ' . $order_id ,$content);
			lib_sendEmail(EMAIL_CLIENTE,'Nueva Cotizacion No. ' . $order_id ,$content_admin);
...
function MCart_SetOrderStatus($orderId,$status){
	...
	if(!empty($order) && $order['estatus'] != $status){
		...
		$content = '
		Se ha modificado el estatus de su cotizaci&oacute;n No. '.$orderId.'
		<br >
		El Nuevo estatus de su cotizaci&oacute;n es: '.$status.'
		<br >
		Puede ver los detalles actualizados de su cotizaci&oacute;n en cualquier momento desde el siguiente link de Acceso: <a href="'. ABS_HTTP_URL .'user.php?cmd=view_order&id='.$orderId.'">'. ABS_HTTP_URL .'user.php?cmd=view_order&id='.$orderId.'</a><br /><br />
		';
		
		$content_admin = '
		Se ha modificado el estatus de su cotizaci&oacute;n No. '.$orderId.'
		<br >
		El Nuevo estatus de la cotizaci&oacute;n es: '.$status.'
		<br >
		Puede ver y modificar los detalles actualizados de la cotizaci&oacute;n en cualquier momento desde el siguiente link de Acceso: 
