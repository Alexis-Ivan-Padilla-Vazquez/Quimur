...
function uploadFile( $File, $path , $validFiles = array(), $maxSize=0 ){
	...
	if( !empty($file) ){
		...
		if(!empty($maxSize) && $file_size > $maxSize){return -2;}
		if( empty($validFiles) || $key ){
			if(!empty($file_err)){return 0;}
...
function deleteFile($imgAnterior){
	if( !empty($imgAnterior) && file_exists($imgAnterior) ){@unlink( $imgAnterior);}
}
function getImgDir($class='',$field='',$w=0,$h=0){
	...
	switch($class){
		default:break;
		case 'galeria':$dir .= 'galerias/';break;
		case 'foto':$dir .= 'fotos/';break;
		case 'contenido': $dir .='Contenido/'; break;
		case 'PlecaRotativa': $dir .='PlecaRotativa/'; break;
		
	}
	return $dir;
}

function getAllImages($id,$seccion){
	$extraConditions = array();     
    $extraConditions[] = array('condition'=>"sec_id = '%s'", 'condition_values'=>array($id));            
    $extraConditions[] = array('condition'=>"seccion = '%s'", 'condition_values'=>array($seccion));            
 
    return $users = DB_INTERFACE_Select('imas',array('*'), $extraConditions, array( 'order'=>'id', 'order_values'=>array('DESC') ), 1, -1); 
}

function delImage($id){
	return delete('imas', "id = '".$id."'");
}
...
function lib_sendEmail($to,$subject,$content){
	$body = '<img src="'.ABS_HTTP_URL.'images/logo2.png"><br /><br />';
		$body .= $content;
	$body .= '<br /><br /><strong style="color:#AA0000">'.PROJECT_NAME.'</strong><br />
...
function mail_smtp($email_to,$subject,$body, $from_name='', $from='', $password=''){
	$from_name = empty($from_name) ? PROJECT_NAME : $from_name;
	$smtp_server = MAIL_SMTP;
	
	$from = empty($from) ? MAIL_SMTP_FROM : $from;
	$password = empty($password) ? MAIL_SMTP_PASSWORD : $password; 
